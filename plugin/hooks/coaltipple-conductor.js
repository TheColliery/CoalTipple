...
function contract(cfg) {
  return [
    '[CoalTipple] Model/effort routing active. Before delegating, ensure a valid model-tier ranking exists (it is the alias floor haiku<sonnet<opus<fable + your modelTiers pins; fable = the top rung, a real-money spawn you ASK before using [once/always-this-project/no], else cap at the rung BELOW fable as READ from the ranking, never a hardcoded model; rebuild it via the coaltipple skill if missing/corrupt). Then:',
    ...
    '- ESCALATE-UP a task beyond the current tier for quality. A worker that fails RETURNS its result and the MAIN re-routes. Spawning MORE THAN ONE worker in one move needs an explicit user yes first (ASK 2: state how many x tier x effort, capped at maxConcurrentSubagents).',
    ...
    langLine(cfg),
    '- Consent + token spend: honor .coaltipple.json for routing spend. Never spawn fable without consent (ASK 1); never fan out several workers silently (ASK 2); near a quota edge, collapse to inline-self instead of fanning out (P12).',
  ].join('\n');
}
