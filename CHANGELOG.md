...
### Added
- **A present-but-unreadable project or global config now says why it was skipped, not just that it was.** Before this, a config file that exists but cannot be read as one—malformed JSON, a directory, a permissions error, or valid JSON that is not an object—was silently treated the same as no config at all. The conductor's session-start report now names the reason next to the path: `UNREADABLE: <path> exists but is not a readable config (<reason>); it was skipped — canonical = .claude/coal/coaltipple.json`, with `<reason>` one of `malformed JSON` · `a directory` · `unreadable` (a permissions error) · `not a JSON object`. A leading BOM is stripped before the parse. The `UNREADABLE` line takes precedence over the existing `LEGACY:` line on the same broken file (a file that failed to parse was never actually read, so `LEGACY`'s own claim would be a second false statement); the global config (`~/.claude/.coaltipple.json`) is reported independently—its own `UNREADABLE:` line names its own path and the same `canonical = .claude/coal/coaltipple.json` wording as the project line. The README's Configure section carries the one sentence naming this line.
...
- `configure.mjs --project` no longer risks corrupting a config that carries JSONC comments: the edit locates the real closing brace outside comment/string text and validates the full result before writing.
- `configure.mjs --project`'s write now goes through a temp file and a rename instead of overwriting the target directly, so a process killed mid-write cannot leave the config empty or truncated; the temp file is acquired exclusively, so a symlink pre-planted at its name is refused rather than followed.
- `configure.mjs --project` now refuses a parsed config that is valid JSON but not a plain object (an array, a bare string, a number) instead of silently adopting it.
...
### Fixed
- **R9 — `skills/coaltipple/SKILL.md` stops telling a CoalTipple-only agent that an absent CoalBoard means no halt.** The conductor's arbitration `cue` (`hooks/coaltipple-conductor.js`, `b7acdbd`) was corrected: with a stakes signal present, CoalBoard being ABSENT this session no longer means "nothing leads" — it means the work still halts and asks the user before acting, with no absent plugin named as the one that leads; the sensitive hard gate then governs routing afterward as usual (never-down, escalate up). `SKILL.md:48` (P13) and `:181` still said "no CoalBoard installed → this rule is INERT," which is exactly the stale reading the hook fix closed. Both rewritten to the halt-and-ask wording; the CoalBoard-present half (CB leads, CT becomes its tier-lever, one consent) is unchanged. Keyed on "present this session" (its hook fired or its skill is listed), never "installed," matching the cue's own test word for word. **The halt is itself a real ask of the user, so it is registered as ASK 4 in the Asks ledger** (`### Asks — exactly 3` → `exactly 4`), never left as unaccounted prose beside a ledger that claims completeness. P-numbering intact at 17 P-rules; the Asks ledger now counts 4. A grep sweep (`stand-down|double-hook|arbitrat|tier-lever` across `README.md`, `commands/*.md`, `skills/coaltipple/SKILL.md`) found no other restatement of the arbitration outside these two sites; the `[1.2.0]` CHANGELOG history describing what shipped then is untouched.
- **R2 — `PRIVACY.md`'s self-update bullet matches `commands/update.md`'s actual per-mode behavior.** Both clauses were wrong: the opening line implied the check runs on the manual command or the `auto` cadence, without saying `ask` and `off` never fire it either way, and the closing clause claimed a manual `/coaltipple update` always runs the check "whatever `updateMode` is set to" — false for `ask` (presents the 3-way choice, saves the pick, no check) and `off` (no update activity, ever). Rewritten so every clause matches the mode table exactly: the check fires under `auto` (its cadence, or a manual run) and under `remind` on a manual run only; `ask` and `off` never fire it, manual or not. The "plain read, no user data" substance is unchanged. The opening sentence (line 3) was also updated to name the manual command and the `auto` cadence explicitly rather than the ambiguous "only when you ask it to."

## [1.5.5] - 2026-09-10

### Fixed
- **CWK-067 — versioned model IDs in shipped text become version-free, across every surface a two-pass sweep found.** `skills/coaltipple/SKILL.md` named `claude-fable-5` as the exemplar concrete Fable id at two rails (ASK 1's trigger description, the decline rail's re-resolve guard) while the live Fable release had already moved past it — not FALSE, but a version-pinned exemplar sitting in text nobody re-pins on release. Both sentences still say exactly what they said before (`isFableModel`'s substring match catches a bare alias AND any pinned concrete release id · a decline blocks the RESOLVED name, not the literal `fable`, or a concrete pin would re-resolve and re-serve after a decline), now with "a pinned concrete Fable release id" in place of the version number. A first sweep pattern-matched only `claude-`-prefixed ids and missed the form that actually dominates this repo, "Fable 5" (capital F, a space, no prefix); a second, wider sweep (`fable[ -]?5|claude-fable`) found it live at six more sites: two `README.md` config-table/permissions bullets, the plugin manifest's own `description` (`.claude-plugin/plugin.json`), the marketplace listing (`.claude-plugin/marketplace.json`), the shipped config template's comment (`platform-configs/.coaltipple.json`), and the `configure.mjs --help` text (`scripts/lib/config-schema.mjs`). All six now say "the fable rung" with no number; the real content at each site (top rung above opus, real-money spawn, ask-once semantics) is unchanged. `scripts/lib/classify.test.mjs`'s three fixtures keep concrete strings (`'Fable 5'`, `'claude-fable-5'`) deliberately — they are what proves `isFableModel`'s substring match catches a real release id, and de-versioning them would delete the test's subject; `classify.mjs:149`'s code comment keeps its concrete example id too, a developer-facing implementation note rather than user-facing ship-text. No rail added, none removed, no mechanism changed — a wording fix whose whole point is that it cannot rot again. Re-pinning the current version was considered and rejected: this room has watched that exact move fail twice already (a `1.3.4` pin that rotted through two releases, corrected once, then rotted again three versions behind — both times caught by an outside reader, never by a gate).
...
### Changed
- **SKILL.md body lean pass — the flock-wide regrowth-ratchet campaign, unit 2 (campaign #6; `scratchpad/longrun/SKILL-BODY-SIZE-2026-08-16.md`).** Measured with the platform's own instrument (`claude plugin details coaltipple`, never `chars/4`), local install pre-synced to source: **~18.4k tokens on-invoke against `skill-authoring.md` §3b's ~5,000-token bar — ~3.7x over**, this room the second-worst-offending whole-product skill in the flock after CoalBoard (which shipped its own lean pass first, v2.3.1, same method). Seven cuts, all EXPLANATION-class per §5's rail/explanation test (does cutting this change what the agent DOES? no → safe to cut) — zero rails touched: Step 1's whole-repo-audit exception trimmed to a bare P10 citation instead of re-deriving the rule · Step 2's HARD GATE paragraph lost a worked example (an under-graded constant-time-compare anecdote) while every rail sentence around it stayed untouched · Step 2's Fable bullet trimmed of content ASK 1's own ledger entry already states in full (the `isFableModel` trigger, the three consent options, the cost-math disclaimer), now opens with a citation to ASK 1 and keeps only what's genuinely Step-2-specific (the cap mechanics, F7's fall-resolution, the sensitive-floor interaction) · three more worked examples cut where the rule they illustrated was already stated in the same bullet (a 10-line-crypto effort example, a 180k-token-codebase context-floor example — its real rail, the large-context-main exception, was kept — and a buggy-`1.5h` verify example) · one sentence of pure competitive-positioning marketing prose with zero behavioral content, the same class this room already corrected once in `README.md` (board #52). **Body 47,641 → 45,790 chars (body-only, frontmatter excluded, LF-normalized — both measurement methods agree), −1,851 (−3.9%).** INSPECT (opus/xhigh) found the seven cuts clean on independent re-derivation — including the Fable bullet, the densest rewrite and the one most likely to have silently dropped a Step-2-only detail: every item the trim removed was verified still stated, verbatim, at ASK 1's ledger entry or elsewhere in the body. **Honest residue accounting, this campaign's own standard (CoalBoard's unit: "if the remainder is rail not fat, ~Nx-the-bar IS the finding — do not force cuts to hit the bar"):** the reviewer scanned everything the seven cuts did not touch — THE FRAME, the full P/F/Ask ledgers, the Grants & denials block, the sensitive HARD GATE's distinct interlocks, the non-English sensitive-gate authority paragraph, Step 3.4's verify + staircase — and found it is rail, the same ledgers the variance walks score. **No further cuts taken to chase the bar; the residue stands as the finding, not a shrug.**

### Notes
- **DECLARED BOUND (skill-authoring.md §3b/§5), matching this campaign's own convention (CoalBoard's unit 1, v2.3.1) — body-only, frontmatter EXCLUDED, LF-normalized: 45,790 chars.** Re-derive: `node -e "const l=require('fs').readFileSync('skills/coaltipple/SKILL.md','utf8').split(/\r?\n/);const e=l.indexOf('---',1);console.log(l.slice(e+1).join('\n').length)"` — confirmed to agree exactly with the LF-normalized/frontmatter-regex-strip method, since this file carries no CRLF. Per §3b, the bound RATCHETS — a future measurement above 45,790 is a regression requiring a re-carve, never a new normal to shrug at. **On-invoke tokens, AFTER, not yet re-measured** — that needs the platform's own instrument against a freshly synced local install, which only happens post-ship; the department head re-runs it once this release propagates.

## [1.5.0] - 2026-08-21

The enumerability carve (variance-walk lab, `TheColliery/scratchpad/longrun/SKILL-VARIANCE-WALK.md` Run 2): SKILL.md rails rehoused into countable ledgers so every "how many / which ones" question has a transcribable answer. No routing mechanic changed — the additions make existing behavior explicit + countable and close the nine walker-measured ambiguities. **This section originally carried a hold — "version moves once the 3-lane variance walk passes" — released here by explicit main/chair order (2026-08-21) ahead of that walk closing.** The hold is a NAMED, DATED exception for this one release, not a repeal: any future accumulation in this room's `[Unreleased]` still waits on the walk unless overridden again the same explicit way.

### Added
- **New `## Grants & denials (CLASSIFY-BLOCK — declared)` section** (`skill-authoring.md` §5b, gold-standard F22): a 3-row read/write/spawn table stating which tool grant each step class needs and what the skill does when one is denied. Closes a gap this room specifically had: a refused `Agent`/`Task`/`Bash` grant on Step 3 used to degrade silently to SELF — byte-identical to the routine, correct SELF outcome — because F1's spawn-fall only ever covered model *unavailability*, never a permission *refusal*; the two looked the same from outside and only one of them meant routing actually ran. Same shape on the write side: a refused persist of ASK 1's `fableConsent` "always" choice used to read as re-asking forever for no stated reason, indistinguishable from a config bug, when it was really a silent write failure. The new section makes both branches explicit — a denial is now something the skill states, never something it quietly becomes SELF or a re-ask instead of. No routing mechanic changed; F1–F11's count is unchanged (grant denials are a separate, permission-level mechanism layered on top, not a twelfth fail-route). Per scripts-quality.md §3's own CHANGELOG-section-drives-SemVer mapping this `### Added` entry is MINOR-shaped, matching the release above.
...
### Security
- **A cloned repo's project config could ESCALATE consent/spend past the user's global choice — MITIGATED for `mode`/`updateMode` at BOTH read sites, at two different strengths; `fableConsent` needed a narrower, different fix and stays weaker still.** The 2-level cascade (`{ ...global, ...project }`) let a project `.coaltipple.json` flip a deliberate global `mode:"off"` to `"auto"`, or `updateMode:"off"` to `"auto"` (standing consent to network + spend tokens). Added a safer-value-wins clamp: a project may only quieten `mode`/`updateMode`, never escalate past an explicit global choice. **The two read sites are NOT equally strong, and the claim must say which is which (`hooks-safety.md` §9):** the HOOK-side clamp — `scripts/lib/config-load.mjs` (`loadMergedConfig`) and the standalone conductor hook (`hooks/coaltipple-conductor.js`, its own Phoenix #9 inline copy) — is **code-enforced: it runs every time, with probability 1, no exceptions.** The SKILL.md-side clamp (new this round: the general merge instruction the AGENT follows when it reads config directly, station-3 review found this was a **second, unclamped decision site** the first round's code-only fix never reached) is **agent-followed: enforced only to the extent the model actually complies with the written rule — real but not guaranteed, and CT has no measured compliance rate for it** (unlike CoalBoard's own ~65% un-primed-prose benchmark for the equivalent instruction — do not borrow that number, it is CoalBoard's, not CT's).
  **`fableConsent` is DIFFERENT, was NOT given the same full-escalation clamp, and its protection is prose-ONLY — there is no code-enforced layer at all for this key.** Its only persistence mechanism is a PROJECT-level write (`configure.mjs --project --fableConsent true`, SKILL.md's "always-this-project" option) with no matching global-write path — clamping a bare project `true` with no global override would silently break that shipped feature (verified live: an early attempt at the fuller clamp broke the "fableConsent persists as a project override" test outright). So `fableConsent` clamps ONLY against an EXPLICIT global `false` — in code (`config-load.mjs`'s `SAFER_FALSE`, unchanged from the first round, reached only by `configure.mjs --list`'s display) and in the SKILL.md instruction the agent actually follows for the real ask-vs-skip decision — but **the conductor hook never reads this key at all, so unlike `mode`/`updateMode` there is no deterministic backstop underneath the prose one.** `hooks-safety.md` §9 names this precisely as a standing, unanswered question ("the bar scales with BLAST... for a gate on real money [an instruction plus an honest claim] is not [proportionate]") — not resolved by this release, named here so it isn't silently assumed closed.
- **RESIDUAL RISK, stated directly (not left to be inferred from the rule's negation): a cloned repo can still set `fableConsent: true` in its project file and make the agent skip the real-money Fable consent ask — IF you have never explicitly set your OWN global `fableConsent` to `false`.** The factory template ships `fableConsent: false` in the GLOBAL file, so most installs that ever ran the configurator are covered; a user who deleted or never had that line is not.
- **The clamp skipped the FACTORY DEFAULT when no global file exists at all — the common case for a fresh install.** Both clamp copies (`config-load.mjs`, the conductor) previously did `if (global[key] === undefined) continue`, giving zero protection to a user who never wrote a global config, even though the schema's declared default (`updateMode: "ask"`, `mode: "auto"`) IS their effective stance. Fixed: an absent global now substitutes the schema default when computing the safer index. `mode`'s default (`auto`) is already the top of its own ordering, so this is a no-op ceiling for that key; `updateMode` gains real protection. `fableConsent` is deliberately excluded from this substitution too, for the same "always-this-project" reason above.
...
- **Test-proof precision, corrected TWICE now (the first correction itself miscounted — see below):** the round-1 commit message claimed all 11 new tests were confirmed failing against the pre-fix code; re-measured, **7 of 11** were genuine red→green proof, the other 4 (the "quieten"/"absent-global" cases) pass either way and are regression guards, not discriminators. **This round's own CHANGELOG entry then repeated the same mistake in the opposite direction — it claimed "5 more tests... all individually red→green verified" when the actual diff is: 1 test removed + 3 added in `config-load.test.mjs`, + 1 added in `conductor-update.test.mjs` = net +3 (165→168, matching exactly), and only 2 of those 4 new tests are red→green proof** (the `updateMode` schema-default clamp, at both the `config-load.mjs` unit level and the conductor's own spawned-hook level, each verified via a `git stash`/re-run round-trip). The other 2 (`mode`'s ceiling no-op, `fableConsent`'s preserved pass-through) are regression guards documenting unchanged behavior, not new coverage — this round's own commit message had the count right; this CHANGELOG entry did not, until now.
