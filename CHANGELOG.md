...
### Security
- **A cloned repo's project config could ESCALATE consent/spend past the user's global choice — closed for `mode`/`updateMode` at BOTH read sites; `fableConsent` needed a narrower, different fix.** The 2-level cascade (`{ ...global, ...project }`) let a project `.coaltipple.json` flip a deliberate global `mode:"off"` to `"auto"`, or `updateMode:"off"` to `"auto"` (standing consent to network + spend tokens). Added a safer-value-wins clamp: a project may only quieten `mode`/`updateMode`, never escalate past an explicit global choice — applied identically in `scripts/lib/config-load.mjs` (`loadMergedConfig`) AND the standalone conductor hook (`hooks/coaltipple-conductor.js`, its own Phoenix #9 inline copy) AND (new this round) the general merge instruction in `skills/coaltipple/SKILL.md` that the AGENT itself follows when reading config directly — station-3 review found the SKILL.md read path was a **second, unclamped decision site** the first round's code-only fix never reached, and required naming, not just coding around.
  **`fableConsent` is DIFFERENT and was NOT given the same full-escalation clamp, by design, not by omission:** its only persistence mechanism is a PROJECT-level write (`configure.mjs --project --fableConsent true`, SKILL.md's "always-this-project" option) with no matching global-write path — clamping a bare project `true` with no global override would silently break that shipped feature (verified live: an early attempt at the fuller clamp broke the "fableConsent persists as a project override" test outright). So `fableConsent` clamps ONLY against an EXPLICIT global `false` (never let a project override a global you deliberately set to false) — both in code (`config-load.mjs`'s `SAFER_FALSE`, unchanged from the first round) and now in the SKILL.md instruction the agent actually follows for the real ask-vs-skip decision (the conductor never reads this key at all, so its clamp was always code-side moot for `fableConsent` specifically).
...
- **Test-proof precision, corrected:** the first round's commit message claimed all 11 new tests were confirmed failing against the pre-fix code. Re-measured: **7 of 11** were genuine red→green proof (2 pre-existing tests plus the "quieten"/"absent-global" pass-either-way cases were regression guards, not discriminators — real value, but not evidence of the escalation bug). This round adds 5 more tests (2 in `config-load.test.mjs`, 1 in `conductor-update.test.mjs`, 2 rewritten to preserve "always-this-project"), all individually red→green verified via `git stash`/re-run round-trips.
