...
* **Reproducible Builds:** Run `node scripts/build-plugin.mjs` to generate a plugin distribution that matches source byte-for-byte, except `.js`/`.json`/`.md` files (today's shipped extensions) are compared EOL-normalized — a CRLF-vs-LF checkout of identical content is not flagged as drift, but any other content difference still is.
