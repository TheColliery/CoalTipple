...
<!-- version-transition: the pin below reflects the LAST ACTUAL scan -- do NOT bump the scanner/CoalTipple version or score without a real re-scan (an unscanned version's security is UNVERIFIED; never claim coverage). Re-scan periodically or on a significant SKILL.md change, then re-sync. Findings reference the SKILL.md section by NAME (not a line number) so they do not drift on a skill edit. Last scan: SkillSpector v2.3.5, CoalTipple v1.0.20 (commit 7e21011), 2026-06-24, static stage -- score 43/100 (all false-positive), 8 issues (RA1 self-update x8; the RA2 memory-anchor no longer fires). v2.3.x doc-FP-suppression eased the score (v2.2.3 100 -> v2.3.1 51 -> v2.3.5 43). Authoritative report: skillspector-20260624.json (local). -->
## 🔬 Independent Scanning — NVIDIA SkillSpector

CoalTipple is evaluated against [NVIDIA SkillSpector](https://github.com/NVIDIA/skillspector) v2.3.5. **Last scan: CoalTipple v1.0.20 (commit `7e21011`), 2026-06-24.** Scanning is event-driven (a new SkillSpector version, or a genuinely new attack surface) — this pins the last version actually verified.

* **Static Scan (43/100 · 8 issues, all false-positive):** consent-gated **Self-Updating** (v1.0.13) is flagged by the static **RA1 self-modification** rule ×8 (the `/coaltipple:update` command + the conductor's self-update scheduler + `self-update` comments) — the hook only SCHEDULES (no network), the agent offers the platform's own `claude plugin update`; the skill never rewrites its own files. (Under SkillSpector **v2.3.5** the score is **43/100**: per-rule diminishing-returns scoring stops the `RA1` self-update ×8 from saturating to 100, and the v2.3.x doc-FP-suppression PRs eased it further [v2.2.3 100 → v2.3.1 51 → v2.3.5 43]; the `RA2` memory-anchor finding no longer fires.)
